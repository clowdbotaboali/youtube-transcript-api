...
const targetPath = path.join(process.cwd(), 'api', 'index.js');
...
if (!content.includes('/api/paymob/create-checkout')) {
  // Use a targeted indexOf replacement instead of regex!
  const chatIndex = content.indexOf("if (pathname === '/api/chat/clear'");
  if (chatIndex > 0) {
    const before = content.slice(0, chatIndex);
    const after = content.slice(chatIndex);
    content = before + paymobRoutes + "\\n    " + after;
    fs.writeFileSync(targetPath, content);
    console.log('Successfully written Paymob endpoints!');
  } else {
    console.log('Could not find chat/clear index :(');
  }
} else {
